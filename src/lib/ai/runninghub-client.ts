...
export async function submitAngleSwitchTask(
  ...
  if (!baseUrl) {
    throw new Error('Chưa cấu hình RunningHub Base URL');
  }
  if (!appId) {
    throw new Error('Chưa cấu hình RunningHub App ID');
  }
  ...
  try {
      ...
      if (!response.ok) {
        ...
        const error = new Error(
          response.status === 401 || response.status === 403
            ...
            : response.status >= 500
              ? 'RunningHub tạm thời không khả dụng'
              : errorMessage
    ...
    throw new Error('Gửi tác vụ RunningHub thất bại');
  }
}
...
export async function pollTaskUntilComplete(
  ...
  throw new Error('Chuyển đổi góc nhìn quá thời gian, vui lòng thử lại');
}
