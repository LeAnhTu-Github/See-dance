...
async function toUploadFile(imageData: string, name?: string): Promise<{ blob: Blob; filename: string }> {
  ...
  if (isHttpUrl(imageData)) {
    ...
    if (!response.ok) {
      throw new Error(`Tải ảnh xuống thất bại: ${response.status}`);
    }
...
async function uploadWithProvider(
  ...
  try {
    ...
    if (!uploadUrl) {
      return { success: false, error: 'Chưa cấu hình địa chỉ tải lên kho ảnh' };
    }
    ...
    if (!response.ok) {
      ...
      return { success: false, error: `Kho ảnh ${provider.name} tải lên thất bại: ${message}` };
    }
    ...
    return { success: false, error: `Kho ảnh ${provider.name} tải lên thành công nhưng không trả về URL` };
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Tải lên thất bại';
    return { success: false, error: `Kho ảnh ${provider.name} yêu cầu thất bại: ${message}` };
  }
}
...
export async function uploadToImageHost(
  ...
  if (!providers || providers.length === 0) {
    return { success: false, error: 'Chưa cấu hình kho ảnh' };
  }

  const orderedProviders = getRotatedProviders(providers);
  let lastError = 'Tải lên thất bại';
  ...
  for (const provider of orderedProviders) {
    ...
    if (keys.length === 0) {
      if (provider.apiKeyOptional) {
        for (let attempt = 0; attempt < 2; attempt++) {
          ...
          lastError = result.error || 'Tải lên thất bại';
          if (attempt < 1) {
            await sleep(600);
          }
        }
      ...
      lastError = `Kho ảnh ${provider.name} chưa cấu hình API Key`;
      continue;
    }
    ...
    for (let i = 0; i < maxRetries; i++) {
      ...
      if (!apiKey) {
        lastError = 'Tất cả API Key tạm thời không khả dụng';
        break;
      }
