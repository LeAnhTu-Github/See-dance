...
export function Dashboard() {
  ...
  const handleDuplicate = useCallback(async (projectId: string) => {
    ...
    try {
      ...
      if (!fs) {
        toast.warning('Không thể dùng lưu trữ tệp, chỉ sao chép được tên dự án');
      ...
      if (copiedCount > 0) {
        toast.success(`Đã sao chép dự án "${source.name}" (${copiedCount} tệp dữ liệu)`);
      } else {
        toast.warning('Tệp dữ liệu dự án trống, chỉ sao chép được tên dự án');
      }
            ...
            {selectionMode && (
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  Xoá mục đã chọn ({selectedIds.size})
                </Button>
              </div>
            )}
          </div>
