  ...
  if (shots.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="text-xs text-zinc-600 mt-1">Phân tích kịch bản rồi bấm "Tạo danh sách phân cảnh"</p>
      </div>
    );
  }
    ...
    >
      ...
      <div className="flex items-center justify-between mb-1">
        ...
        <div className="flex items-center gap-1">
          {shot.imageUrl && (
            <span className="w-2 h-2 rounded-full bg-green-500" title="Đã tạo ảnh" />
          )}
          {shot.videoUrl && (
            <span className="w-2 h-2 rounded-full bg-blue-500" title="Đã tạo video" />
          )}
        </div>
      </div>
