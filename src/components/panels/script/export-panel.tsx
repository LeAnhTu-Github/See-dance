...
export function ExportPanel({ projectName, scriptData, shots, targetDuration }: ExportPanelProps) {
  ...
  const handleExportToFolder = async () => {
    if (!stats.canExport) {
      toast.error('Không có tư liệu để xuất');
      return;
    }

    setIsExporting(true);
    setProgress({ current: 0, total: 0, message: 'Đang chuẩn bị xuất...' });
  ...
  const handleDownloadFiles = async () => {
    ...
    setIsExporting(true);
    setProgress({ current: 0, total: 0, message: 'Đang chuẩn bị tải xuống...' });
  ...
  return (
    <div className="space-y-4 p-4 rounded-lg border bg-card">
      <div className="flex items-center justify-between">
        <h3 className="font-medium text-sm">Gói xuất tư liệu</h3>
        <span className="text-xs text-muted-foreground">
          Dùng cho phần mềm chỉnh sửa video như CapCut, PR...
        </span>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="p-2 rounded bg-muted/50">
          ...
          <div className="text-xs text-muted-foreground">Tổng shot</div>
        </div>
        <div className="p-2 rounded bg-muted/50">
          <div className="text-lg font-semibold text-green-500">{stats.imagesReady}</div>
          <div className="text-xs text-muted-foreground">Ảnh sẵn sàng</div>
        </div>
        <div className="p-2 rounded bg-muted/50">
      ...
      <div className="text-xs text-muted-foreground space-y-1">
        <div className="flex items-center gap-1">
          ...
          Kèm tệp metadata manifest.json
        </div>
      ...
      {!stats.canExport && (
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          Vui lòng tạo ảnh hoặc video cho shot trước
        </div>
      )}
    </div>
  );
