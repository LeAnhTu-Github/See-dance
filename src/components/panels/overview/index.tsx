...
const OVERVIEW_WORKFLOW_SECTIONS: Array<{ id: number; title: string; steps: string[] }> = [
  {
    ...
    steps: [
      "Nhấn vào module Kịch bản",
      "Nhập",
      "Dán toàn bộ kịch bản",
      "Nhấn Nhập toàn bộ kịch bản",
      "Chọn phong cách thị giác",
      "Kiểm tra lại",
      "Hiệu chỉnh bối cảnh bằng AI",
      "Nhấn Bối cảnh, thanh phải: vào thư viện bối cảnh để tạo",
      "Ở bảng bên trái của mục Bối cảnh, sau khi tạo thì lưu lại",
      "Hiệu chỉnh phân cảnh bằng AI",
      "Hiệu chỉnh nhân vật bằng AI",
      "Nhấn Nhân vật, thanh phải: vào thư viện nhân vật để tạo hình",
      "Trong mục Nhân vật nhấn \"Tạo thiết kế nhân vật\"",
      "Tạo xong thì lưu lại",
    ],
  },
  {
    id: 2,
    ...
    steps: [
      "Nhấn vào module Đạo diễn",
      "Nhấn thanh cây bên trái",
      "Chọn bối cảnh cần dùng rồi nhấn \"+\" để thêm vào khu biên tập phân cảnh bên trái",
      "Cách tạo ảnh: chọn \"Tạo tổng hợp\", tự chọn tham số và ảnh",
      "Nhấn Thực thi tạo tổng hợp",
      "Quá trình tạo ảnh hoàn tất",
      "Khi không có nhân vật chính: nút \"Tạo ảnh thành phẩm\"",
      "Khi đã có ảnh thành phẩm với nhân vật chính: nút \"Tạo video\" ở dưới mọi trang phân cảnh",
    ],
  },
];
      ...
      <div className="h-full p-6">
        <div className="mx-auto w-full max-w-6xl rounded-xl border bg-panel">
          <div className="border-b px-5 py-4">
            ...
            <h3 className="mt-2 text-lg font-semibold text-foreground">Quy trình cơ bản phiên bản offline</h3>
            <p className="mt-1 text-sm text-muted-foreground">Thực hiện tuần tự, không bỏ qua bước nào.</p>
          </div>
          ...
          <ScrollArea className="h-full">
            <div className="p-4 space-y-4 pb-32">
              ...
              <SectionCard icon={BookOpen} title="Cốt truyện">
                ...
                <FieldRow label="Đề cương">
                  <EditableText value={meta.outline} placeholder="Cốt truyện đầy đủ 100-500 chữ..." onSave={(v) => update({ outline: v })} multiline />
                </FieldRow>
                ...
                <FieldRow label="Chủ đề">
                  <div className="flex flex-wrap gap-1">
                    ...
                    {(!meta.themes || meta.themes.length === 0) && (
                      <span className="text-xs text-muted-foreground italic">Chưa thiết lập thẻ chủ đề</span>
                    )}
                  </div>
                </FieldRow>
              </SectionCard>
              ...
              <SectionCard icon={Globe} title="Thế giới quan">
                ...
                <FieldRow label="Thể loại">
                  <EditableText value={meta.genre} placeholder="Võ hiệp/Thương chiến/Lãng mạn..." onSave={(v) => update({ genre: v })} />
                </FieldRow>
                <FieldRow label="Tuyến thời gian">
                  <EditableText value={meta.timelineSetting} placeholder="Thiết lập tuyến thời gian chính xác..." onSave={(v) => update({ timelineSetting: v })} />
                </FieldRow>
                ...
                <FieldRow label="Ghi chú">
                  <EditableText value={meta.worldNotes} placeholder="Bổ sung thiết lập..." onSave={(v) => update({ worldNotes: v })} multiline />
                </FieldRow>
              </SectionCard>
              ...
              <SectionCard icon={Settings2} title="Thiết lập sản xuất">
                <FieldRow label="Phong cách thị giác">
                  <span className="text-xs">{meta.styleId ? getStyleName(meta.styleId) : "Chưa thiết lập"}</span>
                </FieldRow>
                <FieldRow label="Tông màu">
                  <EditableText value={meta.colorPalette} placeholder="Tông màu chủ đạo của toàn bộ phim..." onSave={(v) => update({ colorPalette: v })} />
                </FieldRow>
              ...
              <SectionCard icon={ListOrdered} title={`Danh mục tập (${episodes.length} tập)`}>
                {episodes.length === 0 ? (
                  <p className="text-xs text-muted-foreground italic">Chưa có dữ liệu tập (sẽ tự tạo sau khi nhập kịch bản)</p>
                ) : (
                  <div className="space-y-2">
                    {episodes.map((ep) => {
            ...
            <div className="p-4 space-y-4 pb-32">
              ...
              <SectionCard icon={Users} title={`Nhân vật (${meta.characters.length})`}>
                ...
                {meta.characters.length > 20 && (
                  <p className="text-[10px] text-muted-foreground">
                    Còn {meta.characters.length - 20} nhân vật khác...
                  </p>
                )}
              </SectionCard>
              ...
              <SectionCard icon={Shield} title={`Phe phái (${meta.factions?.length || 0})`}>
                {!meta.factions?.length ? (
                  <p className="text-xs text-muted-foreground italic">Chưa có dữ liệu phe phái (AI sẽ tự điền sau khi hiệu chỉnh)</p>
                ) : (
                  <div className="space-y-2">
              ...
              <SectionCard icon={Gem} title={`Vật phẩm quan trọng (${meta.keyItems?.length || 0})`}>
                <NamedEntityList
                  items={meta.keyItems}
                  emptyText="Chưa có vật phẩm (AI sẽ tự điền sau khi phân tích)"
              ...
              <SectionCard icon={MapPin} title={`Thiết lập địa lý (${meta.geography?.length || 0})`}>
                <NamedEntityList
                  ...
                  emptyText="Chưa có dữ liệu địa lý (AI sẽ tự điền sau khi phân tích)"
