...
export function ExportView() {
  ...
  const handleExportToFolder = useCallback(async () => {
    ...
    setIsExporting(true);
    setExportProgress({ current: 0, total: 0, message: 'Đang chuẩn bị xuất...' });
    ...
    try {
      ...
      toast.error(`Xuất thất bại: ${(error as Error).message}`);
    } finally {
  ...
  const handleDownloadFiles = useCallback(async () => {
    if (isExporting) return;
    setIsExporting(true);
    setExportProgress({ current: 0, total: 0, message: 'Đang chuẩn bị tải xuống...' });
    ...
    try {
      ...
      toast.error(`Tải xuống thất bại: ${(error as Error).message}`);
    } finally {
      setIsExporting(false);
      setExportProgress(null);
    }
          ...
          <div className="max-w-6xl mx-auto space-y-8">
            ...
            <div className="bg-card border border-border rounded-xl p-8 shadow-2xl relative overflow-hidden">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  {isExporting ? (
                    ...
                    <><FolderOpen className="w-4 h-4 mr-2" />Chọn thư mục để xuất</>
                  )}
                </Button>
              ...
              {hasSplitScenes && directorStats && (
                <div className="mt-4 text-xs text-muted-foreground">
                  Có thể xuất: {directorStats.imagesReady} frame đầu · {directorStats.videosReady} video{directorStats.endFramesReady > 0 ? ` · ${directorStats.endFramesReady} frame cuối` : ''}
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              >
                ...
                <div>
                  <h4 className="text-sm font-bold text-foreground mb-1">Tải tư liệu</h4>
                  <p className="text-[10px] text-muted-foreground">
                    Tải tất cả ảnh và video đã tạo
                  </p>
                </div>
              </div>
