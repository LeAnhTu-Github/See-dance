    ...
    if (data.name && data.location) {
      ...
      if (data.viewpoints && data.viewpoints.length > 0 &&
        ...
        toast.success(
          ...
          `✔ Đã tải ${data.viewpoints.length} góc nhìn${pageCount > 1 ? ` (${pageCount} ảnh ghép)` : ''}`
        );
      } else {
        toast.success(`Bối cảnh "${data.name}" đã được tạo tự động`);
      }
    } else {
      // 只有部分数据，仅填充表单
      setName(data.name || "");
...
风格：${stylePreset?.name || '动画风格'}，所有格子光照一致，格子之间用细白边框分隔，只有背景，没有人物。`;
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      toast.success("Đã tạo ảnh concept bối cảnh, vui lòng xem trước và xác nhận");
    } catch (error) {
  ...
  const handleGenerateContactSheetPrompt = () => {
    ...
    if (sceneShots.length === 0) {
      toast.warning("Bối cảnh này không có phân cảnh liên kết, sẽ dùng góc nhìn mặc định");
    }
  ...
  const handleCopyPrompt = (isEnglish: boolean) => {
    ...
    if (isEnglish) {
      ...
      fullPrompt = [
        ...
        `Từ khoá phong cách: ${styleTokens}`,
    ...
    toast.success(isEnglish ? "Đã sao chép prompt tiếng Anh (gồm phong cách và tỉ lệ)" : "Đã sao chép prompt tiếng Trung (gồm phong cách và tỉ lệ)");
  };
  ...
  const handleGenerateContactSheetImage = async () => {
    ...
    try {
      ...
      toast.success("Tạo ảnh ghép thành công, có thể tiến hành cắt");
    } catch (error) {
      const err = error as Error;
  ...
  const handleUploadContactSheet = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      toast.success("Đã tải lên ảnh ghép, có thể tiến hành cắt");
    };
  ...
  const handleDirectUploadContactSheet = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const autoSceneName = `Cảnh ảnh tổng hợp-${timestamp}`;
    ...
    reader.onload = (event) => {
      const dataUrl = event.target?.result as string;
      setContactSheetImage(dataUrl);
      toast.success(`Đã tải lên ảnh ghép (${dims.rows}×${dims.cols} = ${totalCells} ô), sau khi cắt sẽ tự tạo bối cảnh mới`);
    };
    reader.readAsDataURL(file);
  };
  ...
  const handleSaveViewpointImages = async () => {
    ...
    if (!parentScene) {
      // 检查表单数据
      const sceneName = name.trim() || 'Cảnh chưa đặt tên';
    ...
    if (contactSheetImage && contactSheetImage.startsWith('data:')) {
      ...
      if (csPath.startsWith('local-image://')) {
        ...
        addMediaFromUrl({
          url: csPath,
          name: `Ảnh tổng hợp-${parentSceneName}`,
          type: 'image',
          source: 'ai-image',
          folderId: csAiFolder,
          projectId: parentScene.projectId ?? resourceProjectId ?? undefined,
    ...
    toast.success(`Đã tạo ${createdVariantIds.length} cảnh biến thể góc nhìn`);
  ...
  const handleAutoGenerateContactSheet = async () => {
    ...
    const snapshotName = name.trim() || selectedScene?.name || 'Cảnh chưa đặt tên';
    ...
    toast.info(`Cảnh "${snapshotName}" bắt đầu tạo ảnh tổng hợp...`);
    ...
    (async () => {
      try {
        ...
        setContactSheetTask(parentSceneId, { status: 'generating', progress: 30, message: 'Đang gọi AI để tạo...' });
        ...
        if (!parentScene) {
          throw new Error('Cảnh cha đã bị xoá');
        }
        const parentSceneName = parentScene.name || parentScene.location;
        const targetFolderId = parentScene.folderId;
        const createdVariantIds: string[] = [];
        ...
        setContactSheetTask(parentSceneId, { status: 'done', progress: 100, message: `Hoàn tất, đã tạo ${createdVariantIds.length} cảnh con` });
        if (createdVariantIds.length > 0) {
          toast.success(`Ảnh tổng hợp của cảnh "${parentSceneName}" đã được cắt và lưu, tổng cộng ${createdVariantIds.length} cảnh con góc nhìn (nhấp để mở rộng xem)`);
        } else {
          toast.warning(`Ảnh tổng hợp của cảnh "${parentSceneName}" đã lưu, nhưng không thể tạo cảnh con (kết quả cắt: ${splitResults.length})`);
        }
        ...
        toast.error(`Tự động tạo ảnh tổng hợp cho cảnh thất bại: ${err.message}`);
        // 10秒后清除错误状态
        setTimeout(() => {
          setContactSheetTask(parentSceneId, null);
        }, 10000);
  ...
  const handleBatchGenerateOrthographic = async () => {
    if (savedChildSceneIds.length === 0) {
      toast.error("Không có cảnh con để xử lý");
      return;
    }
    ...
    toast.info(`Bắt đầu tạo bốn góc nhìn cho ${childScenes.length} cảnh con...`);
...
No characters, empty environment.`;
    ...
    setSavedChildSceneIds([]);
    toast.success(`Hoàn tất tạo hàng loạt bốn góc nhìn! Thành công ${successCount}, thất bại ${failCount}`);
  };
...
使用平坦、中性的电影光照以确保纹理可见。无角色，空场景。`;
  ...
  const handleGenerateOrthographicImage = async () => {
    ...
    try {
      ...
      toast.success("Tạo bốn góc nhìn thành công, có thể tiến hành cắt");
    } catch (error) {
      const err = error as Error;
  ...
  const handleUploadOrthographic = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      toast.success("Đã tải lên bốn góc nhìn, có thể tiến hành cắt");
    };
    reader.readAsDataURL(file);
  };
      ...
      <div className="h-full flex flex-col">
        <div className="p-3 pb-2 border-b flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-medium text-sm">Bốn góc nhìn (trực giao)</h3>
          </div>
        ...
        <ScrollArea className="flex-1 p-3">
          <div className="space-y-4">
            ...
            {(() => {
              ...
              if (overviewImage) {
                referenceImages.push({ label: 'Tham chiếu toàn cảnh', src: overviewImage });
              }
              ...
              return (
                <div className="space-y-2">
                  <Label className="text-xs">Ảnh tham chiếu (tự động)</Label>
                  <div className="grid grid-cols-2 gap-2">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    💡 Dùng cảnh con "Toàn cảnh" làm tham chiếu chính để đảm bảo bốn góc nhìn đồng nhất phong cách
                  </p>
                </div>
              );
            })()}
              ...
              <div className="space-y-2">
                ...
                <label className="block">
                  ...
                  <div className="flex items-center justify-center gap-2 p-2 border border-dashed rounded-lg cursor-pointer hover:border-primary hover:bg-muted/50 transition-colors">
                    ...
                    <span className="text-xs text-muted-foreground">Tải ảnh có sẵn</span>
                  </div>
                </label>
              </div>
            )}
            ...
            <details className="group" open>
              <summary className="flex items-center gap-1 text-xs text-muted-foreground cursor-pointer hover:text-foreground">
                ...
                Prompt bốn góc nhìn (có thể chỉnh sửa, dùng trực tiếp để tạo)
              </summary>
                  ...
                  return (
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        <Label className="text-xs">Prompt tạo ({langLabel}, chỉnh sửa sẽ dùng trực tiếp)</Label>
                        <Button
                          variant="ghost"
                          size="sm"
        ...
        <div className="p-3 border-t">
          <p className="text-xs text-muted-foreground text-center">
            💡 Bốn góc nhìn đảm bảo cảnh nhất quán không gian ở các góc máy khác nhau
          </p>
        </div>
      </div>
    );
  ...
  if (contactSheetPrompt) {
    ...
    return (
      <div className="h-full flex flex-col">
        ...
        <ScrollArea className="flex-1 p-3">
          <div className="space-y-4">
            ...
            <details className="group" open>
              <summary className="flex items-center gap-1 text-xs text-muted-foreground cursor-pointer hover:text-foreground">
                <span className="group-open:rotate-90 transition-transform">▶</span>
                Prompt ảnh tổng hợp (có thể chỉnh sửa, dùng trực tiếp để tạo)
              </summary>
              <div className="mt-2 space-y-2">
                {(() => {
                  const effectiveLang = promptLanguage || scriptProject?.promptLanguage || 'zh';
        ...
        <div className="p-3 border-t">
          <p className="text-xs text-muted-foreground text-center">
            💡 Sau khi nhấp "Tạo ảnh tổng hợp", hệ thống tự động cắt và lưu, có thể khởi tạo nhiều tác vụ liên tiếp
          </p>
        </div>
      </div>
    );
  ...
  if (previewUrl) {
    return (
      <div className="h-full flex flex-col p-3">
        <h3 className="font-medium text-sm mb-3">Xem trước concept art của cảnh</h3>
        <ScrollArea className="flex-1">
          <div className="space-y-4">
            <div className="relative rounded-lg overflow-hidden border-2 border-amber-500/50 bg-muted">
              <img 
                src={previewUrl} 
                alt="Xem trước concept art của cảnh"
                className="w-full h-auto"
              />
  ...
  return (
    <div className="h-full flex flex-col">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Ví dụ: Đường phố, Nhà gỗ trong rừng"
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="Mô tả chi tiết môi trường cảnh, ví dụ: Ngã tư Shibuya Tokyo sầm uất, đèn neon nhấp nháy..."
          ...
          <div className="space-y-2">
            ...
            <p className="text-[10px] text-muted-foreground">
              AI sẽ dùng những ảnh này làm tham chiếu để tạo concept art của cảnh
            </p>
          </div>
        </div>
      </ScrollArea>
      ...
      <div className="p-3 border-t space-y-2">
        ...
        {savedChildSceneIds.length > 0 && (
          <div className="p-3 rounded-lg border-2 border-dashed border-primary/50 bg-primary/5 space-y-2">
            <div className="text-xs text-center">
              <span className="font-medium">Đã lưu {savedChildSceneIds.length} cảnh con</span>
              <p className="text-muted-foreground">Có thể tạo bốn góc nhìn cho mỗi cảnh con (tổng {savedChildSceneIds.length * 4} ảnh)</p>
            </div>
            ...
            >
              {isGenerating ? (
                ...
                <>
                  ...
                  {selectedScene.referenceImage ? 'Tạo lại concept art' : 'Tạo concept art của cảnh'}
                </>
              )}
            </Button>
          )
        ...
        {generationMode === 'contact-sheet' && (
          <div className="space-y-2">
            ...
            <label className="block">
              ...
              <div className="flex items-center justify-center gap-2 p-2 border border-dashed rounded-lg cursor-pointer hover:border-primary hover:bg-muted/50 transition-colors">
                <Upload className="h-3 w-3 text-muted-foreground" />
                <span className="text-xs text-muted-foreground">Tải thẳng ảnh tổng hợp để cắt</span>
              </div>
            </label>
          </div>
        )}
        ...
        <p className="text-xs text-muted-foreground text-center">
          {generationMode === 'single' && '💡 Chế độ ảnh đơn: Tạo concept art một góc nhìn duy nhất'}
          {generationMode === 'contact-sheet' && '💡 Chế độ ảnh tổng hợp: Tạo lưới đa góc nhìn 2x3'}
          {generationMode === 'orthographic' && '💡 Chế độ bốn góc nhìn: Tạo góc trực giao trước/sau/trái/phải'}
        </p>
      </div>
    </div>
  );
