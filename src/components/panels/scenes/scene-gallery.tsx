...
export function SceneGallery({ onSceneSelect, selectedSceneId }: SceneGalleryProps) {
  ...
  const handleDeleteFolder = (id: string) => {
    if (confirm("Bạn chắc chắn muốn xoá thư mục này? Bối cảnh trong thư mục sẽ được chuyển lên thư mục cha.")) {
      deleteFolder(id);
                ...
                return (
                  <SceneContextMenu
                    ...
                    onMove={(folderId) => {
                      ...
                      toast.success("Đã di chuyển bối cảnh");
                    }}
                  >
                    <SceneCard
                      scene={scene}
          ...
          subFolders.length === 0 && (
            <div className="flex flex-col items-center justify-center h-[200px] text-center">
              ...
              <p className="text-sm text-muted-foreground">
                {searchQuery ? "Không tìm thấy bối cảnh phù hợp" : "Chưa có bối cảnh nào"}
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                Dùng bảng điều khiển bên trái để tạo bối cảnh
              </p>
            </div>
          )
        )}
    ...
    return (
      ...
      >
        <div
          ...
          title={hasChildren ? (isExpanded ? "Nhấn đúp để thu gọn" : "Nhấn đúp để mở rộng") : "Nhấn đúp để xem ảnh lớn"}
          onDoubleClick={(e) => {
            e.stopPropagation();
            if (hasChildren) {
              // 有子场景时，双击展开/收起而非打开预览
        ...
        >
          ...
          {hasChildren && (
            <div
              ...
              title={isExpanded ? "Thu gọn bối cảnh con" : "Mở rộng bối cảnh con"}
            >
              {isExpanded ? (
