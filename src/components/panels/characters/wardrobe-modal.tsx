...
const VARIATION_PRESETS = [
  { name: "Thường ngày", prompt: "casual everyday clothing, relaxed outfit" },
  { name: "Trang phục trang trọng", prompt: "formal attire, business suit, elegant clothing" },
  { name: "Trang phục chiến đấu", prompt: "tactical gear, combat outfit, armor" },
  { name: "Đồ ngủ", prompt: "sleepwear, pajamas, nightwear" },
  { name: "Đồ thể thao", prompt: "sportswear, athletic clothing, workout outfit" },
  { name: "Bị thương", prompt: "injured appearance, bandages, wounds" },
  { name: "Trang phục đi mưa", prompt: "raincoat, umbrella, wet weather gear" },
...
export function WardrobeModal({ character, open, onOpenChange }: WardrobeModalProps) {
  ...
  const handleDeleteVariation = (variationId: string, name: string) => {
    if (confirm(`Bạn chắc chắn muốn xoá biến thể "${name}"?`)) {
  ...
  const handleGenerateVariation = async (variation: CharacterVariation) => {
    ...
    if (!characterBaseImage) {
      toast.error("Vui lòng tạo ảnh chuẩn của nhân vật trước để đảm bảo nhất quán");
      return;
    }
    ...
    try {
      ...
      toast.success("Đã tạo ảnh biến thể, vui lòng xem trước và xác nhận");
    } catch (error) {
  ...
  const handleSavePreview = async () => {
    ...
    try {
      ...
      addMediaFromUrl({
        url: localPath,
        name: `Tủ đồ-${character.name}-${varName}`,
        type: 'image',
        source: 'ai-image',
        folderId: aiFolderId,
      ...
      setPreviewData(null);
      toast.success("Đã lưu ảnh biến thể vào máy!", { id: 'saving-wardrobe' });
    } catch (error) {
      console.error('[Wardrobe] Failed to save preview:', error);
      toast.error("Lưu thất bại", { id: 'saving-wardrobe' });
    }
  };
        ...
        <DialogContent className="max-w-lg max-h-[80vh] p-0 gap-0 flex flex-col">
          <div className="px-6 pt-6 pb-3 shrink-0">
            <DialogHeader>
              ...
              <DialogDescription>
                Kiểm tra ảnh có ưng ý không, ưng ý thì lưu lại
              </DialogDescription>
            </DialogHeader>
          </div>
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] p-0 gap-0 flex flex-col">
        <div className="px-6 pt-6 pb-3 shrink-0">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Tủ đồ của {character.name}
            </DialogTitle>
            <DialogDescription>
              Quản lý các biến thể tạo hình của nhân vật, AI sẽ giữ đặc điểm khuôn mặt nhất quán khi tạo ảnh
            </DialogDescription>
          </DialogHeader>
        </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden px-6">
          ...
          <div className="flex items-center gap-3 p-3 rounded-lg border bg-muted/30">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] text-muted-foreground bg-muted px-1.5 py-0.5 rounded">Ảnh chuẩn cơ bản</span>
              </div>
              <p className="text-xs text-muted-foreground truncate mt-0.5">
                {character.visualTraits || character.description || 'Chưa thiết lập mô tả thị giác'}
              </p>
              {!characterBaseImage && (
                <p className="text-xs text-amber-600 dark:text-amber-400 mt-1">
                  ⚠ Vui lòng tạo ảnh cơ bản của nhân vật trước, biến thể tủ đồ cần ảnh chuẩn để tham chiếu
                </p>
              )}
            </div>
          </div>
                  ...
                  >
                    <div className="flex items-start gap-3 min-w-0">
                      ...
                      <div
                        ...
                        title={variation.referenceImage ? "Nhấn đúp để phóng to" : undefined}
                      >
          ...
          {showAddForm ? (
            ...
            >
              <Label className="text-sm font-medium">Thêm biến thể tuỳ chỉnh</Label>

              {/* Variation name */}
              <div className="space-y-1">
                <Label className="text-xs text-muted-foreground">Tên biến thể *</Label>
                <Input
                  ref={nameInputRef}
                  placeholder="Ví dụ: váy cưới, áo choàng, đồng phục học sinh, cổ trang"
              ...
              <div className="space-y-2">
                <Label className="text-xs text-muted-foreground">
                  Ảnh tham chiếu trang phục (tuỳ chọn, tối đa {MAX_CLOTHING_REFS} ảnh)
                </Label>
                <p className="text-[11px] text-muted-foreground">
                  Tải lên ảnh trang phục/tạo hình mong muốn, AI sẽ phối nhân vật vào trang phục đó
                </p>
              ...
              {/* Visual prompt */}
              <div className="space-y-1">
                <Label className="text-xs text-muted-foreground">Mô tả thị giác (tuỳ chọn)</Label>
                <Textarea
                  placeholder="Mô tả chi tiết trang phục hoặc tạo hình, ví dụ:\n- Váy cưới ren trắng, đuôi dài, đội vòng hoa\n- elegant white lace wedding dress, long train, floral headpiece"
                  value={newVariationPrompt}
                  onChange={(e) => setNewVariationPrompt(e.target.value)}
                  className="min-h-[72px]"
                />
                <p className="text-[11px] text-muted-foreground">
                  Có thể mô tả bằng tiếng Việt hoặc tiếng Anh, hỗ trợ kết hợp. Khi có ảnh tham chiếu có thể mô tả ngắn gọn để bổ sung chi tiết
                </p>
              </div>
            ...
            >
              ...
              Thêm biến thể tuỳ chỉnh
            </Button>
          )}

          {/* Tips */}
          <div className="text-xs text-muted-foreground space-y-1 pt-2 border-t">
            <p>💡 Biến thể sẽ tham chiếu ảnh chuẩn của nhân vật để giữ đặc điểm khuôn mặt nhất quán</p>
            <p>💡 Tải lên ảnh tham chiếu trang phục để AI tạo tạo hình chính xác hơn</p>
            <p>💡 Nên tạo ảnh cơ bản của nhân vật trước khi thêm biến thể</p>
          </div>
          </div>
        </div>
...
async function pollForVariationImage(
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) throw new Error('Tác vụ không tồn tại');
        continue;
      }
      ...
      if (status === 'completed' || status === 'succeeded' || status === 'success') {
        ...
        throw new Error('Tác vụ hoàn tất nhưng không có URL ảnh');
      }
      ...
      if (error instanceof Error &&
          (error.message.includes('thất bại') || error.message.includes('không tồn tại') || error.message.includes('không có URL'))) {
        throw error;
      }
  ...
  throw new Error('Quá thời gian tạo ảnh');
}
