...
export function CharacterGallery({ onCharacterSelect, selectedCharacterId }: CharacterGalleryProps) {
  ...
  const handleDeleteFolder = (id: string) => {
    if (confirm("Bạn chắc chắn muốn xoá thư mục này? Nhân vật trong thư mục sẽ được chuyển lên thư mục cha.")) {
      deleteFolder(id);
  ...
  const handleDeleteCharacter = (char: Character) => {
    if (confirm(`Bạn chắc chắn muốn xoá nhân vật "${char.name}"?`)) {
          ...
          subFolders.length === 0 && (
            <div className="flex flex-col items-center justify-center h-[200px] text-center">
              ...
              <p className="text-sm text-muted-foreground">
                {searchQuery ? "Không tìm thấy nhân vật phù hợp" : "Chưa có nhân vật nào"}
              </p>
