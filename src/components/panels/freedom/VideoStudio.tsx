...
function getVeoUploadValidationError(
  ...
  if (capability.mode === 'first_last') {
    if (capability.minFiles > 0 && !firstFrameUpload) {
      return 'Model hiện tại cần tải lên ảnh frame đầu';
    }
    if (!firstFrameUpload && lastFrameUpload) {
      return 'Vui lòng tải frame đầu trước, sau đó tải frame cuối';
    }
    return null;
  }
  ...
  if (capability.mode === 'multi') {
    if (referenceUploads.length < capability.minFiles) {
      return `Model hiện tại cần tối thiểu ${capability.minFiles} ảnh tham chiếu`;
    }
    if (referenceUploads.length > capability.maxFiles) {
      return `Model hiện tại tối đa ${capability.maxFiles} ảnh tham chiếu`;
    }
  }
...
export function VideoStudio() {
  ...
  const handleReferenceChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (referenceUploads.length >= Math.max(veoCapability.maxFiles, 1)) {
      toast.error(`Model hiện tại tối đa ${veoCapability.maxFiles} ảnh tham chiếu`);
      return;
    }
    try {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      toast.success('Tạo video thành công! Đã lưu vào thư viện');
    } catch (err: unknown) {
        ...
        <ScrollArea className="flex-1">
          <div className="p-4 space-y-5">
            ...
            {aspectRatios.length > 0 && (
              <div className="space-y-2">
                <Label className="text-sm font-medium">Tỉ lệ khung</Label>
            ...
            {veoCapability.isVeo && (
              <div className="space-y-2">
                <Label className="text-sm font-medium">Tải lên tư liệu (Veo)</Label>
                {veoCapability.mode === 'none' ? (
                  <p className="text-xs text-muted-foreground rounded-md border px-2 py-2">
                    Model hiện tại chỉ hỗ trợ text-to-video, không cần tải ảnh.
                  </p>
                ) : (
                  <div className="space-y-2">
                    ...
                    {veoCapability.mode === 'multi' && (
                      <div className="space-y-2">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Đã tải {referenceUploads.length}/{veoCapability.maxFiles} ảnh tham chiếu
                        </p>
                      </div>
                    )}
                  </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-muted/30">
        {videoGenerating ? (
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-sm text-muted-foreground">Đang tạo video, vui lòng đợi (có thể mất 1-4 phút)...</p>
          </div>
        ) : videoResult ? (
