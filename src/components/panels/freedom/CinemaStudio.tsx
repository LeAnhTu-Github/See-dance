...
export function CinemaStudio() {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      toast.success('Tạo ảnh điện ảnh thành công! Đã lưu vào thư viện');
    } catch (err: any) {
        ...
        <div className="p-4 border-t space-y-3">
          ...
          >
            {cinemaGenerating ? (
              ...
              <><Sparkles className="mr-2 h-4 w-4" /> Chụp điện ảnh</>
            )}
          </Button>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-muted/30">
        {cinemaGenerating ? (
          ...
          <div className="flex flex-col items-center gap-3 text-muted-foreground">
            ...
            <p className="text-xs text-muted-foreground/60">Tham số máy quay sẽ tự động biên dịch thành prompt AI</p>
          </div>
        )}
      </div>
