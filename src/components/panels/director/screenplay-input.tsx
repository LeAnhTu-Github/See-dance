...
const EXAMPLE_PROMPTS = [
  "Một chú mèo dễ thương đang chơi đùa trên bãi cỏ, đuổi theo bướm",
  "Hai người bạn thân đi dạo trong công viên, chia sẻ khoảnh khắc vui vẻ",
  "Chú thỏ và gấu con phiêu lưu trong rừng, khám phá kho báu bí ẩn",
  "Một cô bé xây lâu đài cát bên biển, sóng vỗ nhẹ nhàng",
];
...
export function ScreenplayInput({ onGenerateStoryboard }: ScreenplayInputProps) {
  ...
  const handleSubmit = async () => {
      ...
      try {
        ...
        if (rawCharacterImages.length > 0) {
          ...
          try {
            ...
            toast.warning('Tải ảnh tham chiếu nhân vật thất bại, sẽ không dùng ảnh tham chiếu');
          }
        }
    ...
    if (!chatReady) {
      toast.error('Vui lòng cấu hình ánh xạ dịch vụ cho "Phân tích kịch bản/Chat" trong cài đặt');
      return;
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <Textarea
          placeholder="Ví dụ: Một chú mèo dễ thương đang chơi trên bãi cỏ..."
          value={prompt}
      ...
      {/* Example prompts */}
      <div className="space-y-2">
        <label className="text-xs text-muted-foreground">Gợi ý mẫu</label>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <label className="text-sm font-medium">Ảnh tham chiếu (tuỳ chọn)</label>
      ...
      {!isFeatureConfigured('script_analysis') && !checkChatKeys().isAllConfigured && (
        <div className="flex items-start gap-2 p-2 rounded-md bg-yellow-500/10 border border-yellow-500/20">
          ...
          <div className="text-xs text-yellow-600 dark:text-yellow-400">
            <p className="font-medium">API chưa được cấu hình</p>
            <p className="text-yellow-600/80 dark:text-yellow-400/80">
              Vui lòng cấu hình ánh xạ dịch vụ cho "Phân tích kịch bản/Chat" trong cài đặt
            </p>
          </div>
        </div>
      )}
