...
export function ShotListPanel({ onGenerateImage }: ShotListPanelProps) {
  ...
  const handleBatchGenerate = async () => {
    ...
    if (allStartFramesGenerated && !confirm("Bạn có chắc muốn tạo lại tất cả khung đầu?")) {
      return;
    }
    ...
    return (
      <div className="h-full flex flex-col">
        <div className="p-3 border-b border-border">
          <h3 className="font-medium text-sm">Danh sách shot</h3>
        </div>
        <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground p-4">
          <AlertCircle className="w-10 h-10 mb-3 opacity-30" />
          <p className="text-sm text-center">Chưa có shot</p>
                  ...
                  >
                    ...
                    <div className="p-1.5 bg-background">
                      <p className="text-[10px] text-muted-foreground line-clamp-1">
                        {shot.shotSize || shot.cameraMovement || "Shot"}
                      </p>
                    </div>
                  </div>
                );
