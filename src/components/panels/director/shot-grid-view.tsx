...
export function ShotGridView({ onGenerateImage, onGenerateVideo }: ShotGridViewProps) {
  ...
  const handleGenerateKeyframe = async (shot: Shot, type: "start" | "end") => {
    ...
    try {
      ...
      toast.success(`${type === "start" ? "Khung đầu" : "Khung cuối"} đã tạo xong`);
    } catch (error) {
  ...
  const handleGenerateVideo = async (shot: Shot) => {
    ...
    try {
      ...
      toast.success("Đã tạo video xong");
    } catch (error) {
  ...
  const handleBatchGenerateImages = async () => {
    ...
    if (allStartFramesGenerated) {
      if (!confirm("Bạn có chắc muốn tạo lại khung đầu của tất cả shot?")) return;
    }
    ...
    setBatchProgress({
      ...
      message: allStartFramesGenerated ? "Đang tạo lại tất cả khung đầu..." : "Đang tạo khung đầu hàng loạt...",
    });
    ...
    for (let i = 0; i < shotsToProcess.length; i++) {
      ...
      setBatchProgress((prev) => ({
        ...prev,
        current: i + 1,
        message: `Đang tạo shot ${i + 1}/${shotsToProcess.length}...`,
      }));
  ...
  if (shots.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-zinc-500 bg-[#121212] p-8">
        ...
        <p className="text-sm">Chưa có dữ liệu shot</p>
        <p className="text-xs text-zinc-600 mt-1">Vui lòng tạo danh sách shot ở giai đoạn kịch bản trước</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full bg-[#121212] relative overflow-hidden">
      ...
      <div className="h-14 border-b border-zinc-800 bg-[#1A1A1A] px-4 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-medium text-white">Lưới shot</span>
        ...
        >
          ...
          {allStartFramesGenerated ? "Tạo lại khung đầu" : "Tạo khung đầu hàng loạt"}
        </Button>
      </div>
          ...
          <div className="w-[380px] bg-[#0F0F0F] flex flex-col h-full shadow-2xl animate-in slide-in-from-right-5">
            ...
            <div className="h-14 px-4 border-b border-zinc-800 flex items-center justify-between bg-[#141414] shrink-0">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-white font-medium text-sm">Chi tiết shot</h3>
                ...
                {activeScene && (
                  <div className="bg-[#141414] p-4 rounded-lg border border-zinc-800 space-y-3">
                    <div className="flex items-center gap-2 text-zinc-400">
                      ...
                      <span className="text-xs font-medium uppercase tracking-wider">Bối cảnh</span>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="space-y-1.5">
                      <div className="flex justify-between items-center">
                        <span className="text-[10px] font-medium text-zinc-500">Khung đầu</span>
                        <div className="flex items-center gap-2">
                          {(startKf?.imageUrl || activeShot.imageUrl) && (
                            <button
                    ...
                    <div className="space-y-1.5">
                      <div className="flex justify-between items-center">
                        <span className="text-[10px] font-medium text-zinc-500">Khung cuối</span>
                        <div className="flex items-center gap-2">
                          {endKf?.imageUrl && (
                            <button
                      ...
                      <div className="aspect-video bg-black rounded border border-zinc-800 overflow-hidden relative">
                        {endKf?.imageUrl ? (
                          ...
                          <div className="absolute inset-0 flex items-center justify-center">
                            <span className="text-[9px] text-zinc-700">Tuỳ chọn</span>
                          </div>
                        )}
                        {processingType === "end" && (
                          <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                ...
                <div className="bg-[#141414] rounded-lg p-4 border border-zinc-800 space-y-3">
                  ...
                  {activeShot.videoUrl || activeShot.interval?.videoUrl ? (
                    ...
                    <div className="w-full aspect-video bg-zinc-900/50 rounded border border-dashed border-zinc-800 flex items-center justify-center">
                      <span className="text-xs text-zinc-600 font-mono">Vùng xem trước</span>
                    </div>
                  )}
                  ...
                  {!endKf?.imageUrl && (
                    <p className="text-[9px] text-zinc-600 text-center">
                      * Chưa đặt khung cuối, sẽ dùng chế độ đơn ảnh (Image-to-Video)
                    </p>
                  )}
                </div>
              </div>
