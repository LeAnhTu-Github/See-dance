  ...
  const handleSplit = useCallback(async () => {
    ...
    try {
      ...
      if (storyboardConfig.sceneCount === 1) {
        ...
        toast.success('Đã vào chế độ sửa phân cảnh');
        onSplitComplete?.();
        return;
      }
      ...
      if (splitResults.length === 0) {
        throw new Error("Kết quả cắt rỗng, vui lòng kiểm tra lại ảnh");
      }
      ...
      const splitScenes = await Promise.all(splitResults.map(async (result: SplitResult, index: number) => {
        ...
        return {
          ...
          videoPromptZh: `Phân cảnh ${index + 1}`, // 中文提示词默认值
          needsEndFrame: false,
          endFramePrompt: '',
          endFramePromptZh: '',
          endFrameHttpUrl: null,
      ...
      setStoryboardStatus('editing');
      toast.success(`Đã cắt thành công ${splitScenes.length} phân cảnh`);
      onSplitComplete?.();
    } catch (error) {
  ...
  if (storyboardStatus === 'generating') {
    return (
      <div className="flex flex-col items-center justify-center py-12 space-y-4">
        ...
        <p className="text-sm text-muted-foreground">Đang tạo ảnh ghép storyboard...</p>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium">Đã tạo storyboard</span>
        </div>
      ...
      <div className="flex gap-2">
        <TooltipProvider>
          <Tooltip>
            ...
            <TooltipContent>
              <p>Quay lại giao diện nhập liệu để tạo lại storyboard</p>
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
              ...
              >
                {isSplitting ? (
                  ...
                  <>
                    ...
                    {storyboardConfig.sceneCount === 1 ? 'Bước tiếp' : 'Cắt phân cảnh'}
                  </>
                )}
              </Button>
            </TooltipTrigger>
            <TooltipContent>
              <p>{storyboardConfig.sceneCount === 1 ? 'Đi thẳng vào chỉnh sửa phân cảnh' : 'Cắt theo lưới cố định thành các phân cảnh riêng'}</p>
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
      </div>
      ...
      <div className="text-xs text-muted-foreground bg-muted/50 rounded-md p-2">
        <p>💡 {storyboardConfig.sceneCount === 1
          ? 'Nhấn "Bước tiếp" để vào trực tiếp chỉnh sửa phân cảnh, bạn có thể sửa prompt và tạo video.'
          : `Nhấn "Cắt phân cảnh" để cắt đều theo lưới ${storyboardConfig.sceneCount} ô và tự động bỏ viền ngăn cách. Sau khi cắt, bạn có thể sửa prompt của từng phân cảnh.`
        }</p>
      </div>
    </div>
  );
